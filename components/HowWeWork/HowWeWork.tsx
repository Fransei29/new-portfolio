'use client';

import React, { useEffect, useRef, useState } from 'react';
import Link from 'next/link';
import { HiArrowRight } from 'react-icons/hi';
import styles from './HowWeWork.module.scss';
import { useScrollAnimation } from '../../hooks/Scroll';
import { useLanguage } from '../../contexts/LanguageContext';

interface Step {
  key: string;
  icon: React.ReactNode;
  title: string;
  description: string;
}

const HowWeWork = () => {
  const elementsRef = useScrollAnimation();
  const { t } = useLanguage();

  // El dot NO se desliza continuo: avanza por ETAPAS (paso 1→2→3→4) según el
  // scroll. Se calcula el paso activo y el dot salta a la altura de ese nodo,
  // con la transición suave a cargo de CSS. Lee como "proceso por etapas".
  const gridRef = useRef<HTMLDivElement>(null);
  const nodeRefs = useRef<(HTMLSpanElement | null)[]>([]);
  // Último paso alcanzado (en orden visual). Se guarda en un ref, no en estado:
  // lo usa la histéresis dentro del propio cálculo, y meterlo en el render
  // dispararía un ciclo extra por cada frame de scroll.
  const lastVisualPos = useRef(-1);
  const [activeStep, setActiveStep] = useState(-1); // -1 = aún ninguno
  // Las transiciones se "arman" recién después de la primera medición, para que
  // el panda no se deslice desde el origen al cargar la página.
  const [armed, setArmed] = useState(false);
  const [dotTop, setDotTop] = useState(0);
  // Extremos del riel: primer y último nodo (medidos), para que la línea empiece
  // y termine EXACTAMENTE en los puntos, sin sobrante arriba ni abajo.
  const [railTop, setRailTop] = useState(0);
  const [railBottom, setRailBottom] = useState(0);

  useEffect(() => {
    const el = gridRef.current;
    if (!el) return;

    const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    let raf = 0;
    let ready = false;
    const measure = () => {
      raf = 0;
      // OJO: no compactar el array (nada de filter) — los índices deben seguir
      // correspondiendo 1:1 con los pasos.
      const nodes = nodeRefs.current;
      const count = nodes.length;
      if (!count) return;

      // Línea de "cabezal": el paso se activa cuando su nodo cruza ~68% del alto
      // del viewport. Más arriba que el 0.82 anterior a propósito: a 82% la
      // tarjeta se encendía cuando todavía estaba en el borde inferior, casi
      // fuera de la vista. A 68% el panda llega justo cuando la tarjeta está
      // entrando en la zona que se está mirando.
      const gridTop = el.getBoundingClientRect().top;
      const trigger = window.innerHeight * 0.68;

      // El zigzag usa márgenes negativos, así que el orden del DOM NO coincide
      // con el orden vertical en pantalla. Ordenamos por Y real antes de decidir
      // qué paso está activo; si no, el "último que cruza" da cualquier cosa.
      const measured = nodes
        .map((n, i) => {
          if (!n) return null;
          const r = n.getBoundingClientRect();
          return { index: i, center: r.top + r.height / 2 };
        })
        .filter(Boolean) as { index: number; center: number }[];
      if (!measured.length) return;

      const byY = [...measured].sort((a, b) => a.center - b.center);

      // Avanza mientras el nodo (en orden visual) ya cruzó el cabezal.
      // HISTÉRESIS: para AVANZAR alcanza con cruzar el cabezal; para RETROCEDER
      // hay que volver bastante más abajo (+12% del viewport). Sin este margen,
      // un nodo parado justo sobre la línea alterna activo/inactivo con cada
      // micro-scroll y el panda tiembla entre dos posiciones.
      const RELEASE = trigger + window.innerHeight * 0.12;
      let visualPos = -1;
      for (let i = 0; i < byY.length; i++) {
        const limit = i <= lastVisualPos.current ? RELEASE : trigger;
        if (byY[i].center <= limit) visualPos = i;
        else break;
      }
      if (prefersReduced) visualPos = byY.length - 1;
      lastVisualPos.current = visualPos;

      const activeIdx = visualPos < 0 ? -1 : byY[visualPos].index;
      setActiveStep(activeIdx);

      // Riel: del primer punto al último (en orden visual).
      const first = byY[0].center - gridTop;
      const last = byY[byY.length - 1].center - gridTop;
      setRailTop(first);
      setRailBottom(last);

      const anchor = visualPos < 0 ? byY[0] : byY[visualPos];
      setDotTop(anchor.center - gridTop);

      // Tras la PRIMERA medición se habilitan las transiciones. Si no, el panda
      // se vería viajar desde translateY(0) hasta su sitio al cargar la página:
      // un deslizamiento fantasma que nadie pidió.
      if (!ready) {
        ready = true;
        window.requestAnimationFrame(() => setArmed(true));
      }
    };

    const onScroll = () => {
      if (!raf) raf = window.requestAnimationFrame(measure);
    };

    measure();
    window.addEventListener('scroll', onScroll, { passive: true });
    window.addEventListener('resize', onScroll);
    return () => {
      window.removeEventListener('scroll', onScroll);
      window.removeEventListener('resize', onScroll);
      if (raf) window.cancelAnimationFrame(raf);
    };
  }, []);

  // Giro del panda: UNA VUELTA ENTERA por paso. Con 180° quedaban cabeza abajo
  // los pasos impares (180°, 540°, …); sólo los múltiplos de 360° dejan el
  // isotipo derecho. Como el giro dura lo mismo que el descenso (1.35s), una
  // vuelta completa se percibe igual de bien y el remate siempre queda vertical.
  const pandaSpin = Math.max(0, activeStep) * 360;

  const steps: Step[] = [
    {
      key: 'discovery',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" className={styles.stepIcon}>
          <path fill="currentColor" fillRule="evenodd" d="M7.9 8.47c.359.42.858.519 1.147.839a.63.63 0 0 1 .13.37a.29.29 0 0 0 .29.289h.16q.634.18 1.288.26q.28.03.56 0q.281-.015.558-.07c.39-.07.77-.19 1.159-.3v.779c0 .589-.7.749-1.129.789a5.4 5.4 0 0 1-1.937-.22c.07-.21-.17-.769-.19-.949a.333.333 0 0 0-.659.09c0 .22 0 .849.1 1.099a.45.45 0 0 0 .27.36c.79.299 1.63.445 2.476.428a2.43 2.43 0 0 0 1.708-.709c.137-.167.22-.373.24-.589c0-.26 0-.609.06-.918s.16-.38 1.068-1.189c.462-.54.717-1.226.719-1.937a4.6 4.6 0 0 0-.999-2.746a4.14 4.14 0 0 0-4.633-1.298A4.19 4.19 0 0 0 7.38 6.572c-.026.394.018.79.13 1.169c.08.266.213.513.39.729m.259-1.828a3.335 3.335 0 0 1 2.426-2.816a3.11 3.11 0 0 1 3.435.999c.398.486.663 1.068.77 1.688a2 2 0 0 1-.39 1.637l-.999.929a.5.5 0 0 0-.07.12a.35.35 0 0 0-.17 0c-.429 0-.838-.05-1.257-.06h-1.05c-.399.06-.778.16-1.177.21a1.14 1.14 0 0 0-.21-.46c-.26-.32-.75-.44-1.079-.869a1.55 1.55 0 0 1-.25-.649a2.4 2.4 0 0 1 .02-.729M5.572 4.216q.103.107.22.2q.105.088.22.16c.23.12.45.169.669.299a.29.29 0 1 0 .43-.39a2.9 2.9 0 0 0-.43-.739a1.2 1.2 0 0 0-.26-.23a1.5 1.5 0 0 0-.3-.169a6 6 0 0 0-.738-.25a.335.335 0 1 0-.37.56c.22.19.37.369.56.559M8.448 1.28q.046.158.11.31q.076.144.17.28c.17.209.36.359.55.568a.29.29 0 1 0 .528-.24a7 7 0 0 0-.12-.808A1.7 1.7 0 0 0 9.368.8a6 6 0 0 0-.53-.589a.32.32 0 0 0-.543-.1a.32.32 0 0 0-.035.37q.07.406.19.799m4.362.899a.29.29 0 0 0 .28.3a.3.3 0 0 0 .3-.28q.169-.327.279-.68a1.5 1.5 0 0 0 0-.369c0-.2-.09-.37-.08-.56a.34.34 0 0 0-.2-.429a.33.33 0 0 0-.419.2q-.22.293-.34.64a.9.9 0 0 0-.07.279a.9.9 0 0 0 0 .28q.098.32.25.619m3.396 2.556h1.068l.3-.1c.24-.09.459-.21.708-.31a.33.33 0 0 0 .28-.379a.33.33 0 0 0-.37-.28c-.289 0-.539-.06-.818-.06a1.4 1.4 0 0 0-.35 0a1.4 1.4 0 0 0-.34.11a3.7 3.7 0 0 0-.708.5a.29.29 0 0 0-.086.471a.28.28 0 0 0 .315.048m-6.92 8.278a.3.3 0 0 0-.41 0c-.558.52-1.248.999-1.947 1.538c-.419.32-.828.66-1.288.949q-.71.482-1.508.799a.32.32 0 0 0-.24.4a.33.33 0 0 0 .4.239a7 7 0 0 0 2.127-.71a6 6 0 0 0 .809-.508q.408-.259.779-.57q.726-.8 1.298-1.717a.29.29 0 0 0-.02-.42m2.996.28a.29.29 0 0 0-.28-.28a.3.3 0 0 0-.31.28c-.09.49-.229.999-.299 1.488q-.03.33 0 .659q.006.343.07.679c.087.527.252 1.039.49 1.518a.33.33 0 1 0 .618-.21a7.6 7.6 0 0 1-.1-1.418v-.998c-.03-.6-.13-1.159-.19-1.718m6.352 4.384a.33.33 0 0 0 .46-.08a.32.32 0 0 0-.08-.46a10 10 0 0 1-.81-.788c-.259-.28-.479-.58-.718-.88c-.24-.299-.46-.588-.7-.878c-.499-.619-.998-1.198-1.447-1.807a.293.293 0 1 0-.51.29q.39.999.91 1.937q.24.39.528.749q.287.364.63.679c.526.481 1.11.897 1.737 1.238m-15.079.329A2.786 2.786 0 0 0 .85 20.762c.05 3.116 5.752 3.715 5.542-.2a2.636 2.636 0 0 0-2.836-2.556m-1.767 2.796a1.797 1.797 0 0 1 1.847-1.697a1.637 1.637 0 0 1 1.818 1.538c.379 3.145-3.805 2.436-3.665.14zm12.351.909c-.41-.57-.809-1.598-1.378-2.287a2.5 2.5 0 0 0-1.158-.839a.64.64 0 0 0-.66.28c-.149.23-.479.849-.509.899l-1.867 3.295a.29.29 0 0 0 .301.443a.3.3 0 0 0 .188-.123l2.167-3.076s.28-.25.45-.46q.237.255.42.55c.439.659.808 1.468 1.168 1.997q.245.192.469.41h-.05a3 3 0 0 1-.48.18a5.3 5.3 0 0 1-1.238.209c-.659.05-1.328 0-1.997 0a.34.34 0 0 0 0 .67c.69.049 1.368.129 2.057.139c.484.013.967-.034 1.438-.14a4 4 0 0 0 .77-.23c.216-.098.405-.249.548-.439a.63.63 0 0 0 0-.65c-.18-.229-.589-.758-.639-.828m8.998-.509a5.3 5.3 0 0 0-.27-2.087a1.53 1.53 0 0 0-1.508-.73a7.5 7.5 0 0 0-1.048.1c-.76.12-1.498.32-2.267.4a.34.34 0 0 0-.31.36a.33.33 0 0 0 .36.3c.779 0 1.548-.19 2.316-.26q.47-.039.94 0c.369 0 .568.11.668.3a4.2 4.2 0 0 1 0 1.657q.03.42 0 .839a.75.75 0 0 1-.11.469a.75.75 0 0 1-.39.11c-.389.06-.838.06-1.098.11l-1.128.11h-.789a2.5 2.5 0 0 1-.46 0a.8.8 0 0 1-.319-.09c-.12-.07-.15-.3-.19-.55a12 12 0 0 1-.12-1.378a7 7 0 0 1 0-.779a7 7 0 0 1 0-.769a.292.292 0 0 0-.579-.08q-.07.408-.1.82a8 8 0 0 0 0 .828a9.7 9.7 0 0 0 0 1.738c.037.352.2.678.46.919q.237.167.52.24q.334.09.678.129l.909.05l1.268-.08c.32-.05.879 0 1.318-.13c.281-.054.543-.182.76-.37c.206-.205.342-.47.389-.758c.09-.467.123-.943.1-1.418" clipRule="evenodd"/>
        </svg>
      ),
      title: t('howWeWork.discovery.title'),
      description: t('howWeWork.discovery.description'),
    },
    {
      key: 'design',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" className={styles.stepIcon}>
          <g fill="currentColor" fillRule="evenodd" clipRule="evenodd">
            <path d="M23.116 4.494a6.9 6.9 0 0 0-.63-2.23a11 11 0 0 0-.77-1.281a.75.75 0 0 0-1.061-.15c-.48.41-.87.82-1.34 1.16a3.7 3.7 0 0 1-.901.51c-1.39.58-3.531 1-4.852 1.941a3.07 3.07 0 0 0-1.25 3.461q.09.365.24.71l.15.26a87 87 0 0 0-8.004 5.923c-1.23.96-2.3 2.107-3.17 3.401a6 6 0 0 0-.611 1.661c-.08.426-.08.864 0 1.29a3.53 3.53 0 0 0 1.72 2.392a3.34 3.34 0 0 0 3.002.19A14.5 14.5 0 0 0 9.28 20.33c2.631-3.091 5.702-7.413 6.763-8.733a.41.41 0 0 0-.05-.56a.5.5 0 0 0-.2-.06c-.22-.29-.42-.59-.66-.87a3 3 0 0 0-.35-.36a2.5 2.5 0 0 0-.4-.311a2.16 2.16 0 0 1-1.221-1.68a2.28 2.28 0 0 1 1.05-2.502a7.3 7.3 0 0 1 2.081-.83q.025.229.12.44a1.9 1.9 0 0 0 1 .78c.372.099.715.284 1 .54c.239.306.363.684.351 1.071a3.7 3.7 0 0 0 .44 1.73a2 2 0 0 0 .65.521q-.617.51-1.32.89a4.5 4.5 0 0 1-2.07.56a.39.39 0 0 0-.421.37a.39.39 0 0 0 .37.42a4.8 4.8 0 0 0 2-.35a9 9 0 0 0 2.622-1.51a5.6 5.6 0 0 0 1.18-1.4a7 7 0 0 0 .69-1.671a6.4 6.4 0 0 0 .21-2.321m-8.904 6.553c.26.17.52.31.78.46c-1.32 1.6-4.201 5.302-6.692 8.003a13.5 13.5 0 0 1-3.261 3.001a2 2 0 0 1-1.791-.14a2.17 2.17 0 0 1-1.07-1.47a2.5 2.5 0 0 1 .06-1q.154-.568.42-1.091a18 18 0 0 1 3.861-4.122c2.361-2.07 5.092-4.132 6.393-5.072q.242.415.54.79q.339.375.76.65zm7.533-4.522c-.1.508-.272 1-.51 1.46a4.3 4.3 0 0 1-.69.92a1 1 0 0 1-.48-.4a3 3 0 0 1-.08-1.36a2.77 2.77 0 0 0-.68-1.89a2.84 2.84 0 0 0-1.351-.621c-.32-.07-.65 0-.82-.26a.46.46 0 0 1-.15-.16c.66-.16 1.32-.31 1.86-.5c.438-.154.85-.373 1.22-.65q.522-.379 1.001-.81c.11.26.2.55.21.57c.283.569.472 1.18.56 1.81c.074.631.044 1.27-.09 1.89"/>
            <path d="M1.247 4.484c.46.85-.46-1 4.192 7.563a.35.35 0 0 0 .644-.053a.36.36 0 0 0-.014-.267l-1.09-2.511q.484-.125.93-.35q.405-.22.76-.51c.36-.29.66-.61 1-.9a.394.394 0 1 0-.38-.691q-.672.21-1.31.51a4.5 4.5 0 0 0-1 .6c-.17.14-.33.29-.49.44l-.881-2l.48-.18c.17-.08.34-.17.49-.26s.31-.21.46-.32c.35-.28.67-.57 1.001-.86a.395.395 0 1 0-.4-.681c-.42.15-.83.28-1.25.46q-.261.105-.5.25q-.25.14-.471.32a4 4 0 0 0-.28.28c-.24-.48-.54-1-.78-1.47a5 5 0 0 1-.16-.35q.382-.148.78-.25c.53-.27 1.1-.57 1.67-.84s1.15-.521 1.731-.761q.376-.09.73-.25v.08c.23.34.44.75.61 1c.53.74 1.001 1.52 1.511 2.311s.92 1.61 1.32 2.431a.4.4 0 0 0 .531.2a.4.4 0 0 0 .19-.53c-.36-.86-.78-1.72-1.22-2.561A42 42 0 0 0 8.65 1.863c-.19-.29-.4-.81-.66-1.19a2.2 2.2 0 0 0-.55-.6a.8.8 0 0 0-.66 0c-.2.05-.801.36-.871.39c-.61.23-1.22.48-1.81.74s-1.191.56-1.761.86c-.43.225-.84.49-1.221.79a.77.77 0 0 0-.24.75q.138.462.37.881M17.674 17.99c-.42-.801-.9-1.571-1.33-2.362c-.181-.33-.361-.67-.511-1a.4.4 0 0 0-.74.3l1.55 3.562c.07.17.21.45.32.74q.084.187.11.39q-.489.204-1 .34l-3.091 1.83c-.14.08-.36.221-.59.331l-.24.13a.7.7 0 0 1-.06-.2l-.5-.91q.385-.059.76-.17q.185-.078.36-.18a1.7 1.7 0 0 0 .32-.23c.25-.21.45-.43.69-.66a.4.4 0 0 0 .15-.541a.4.4 0 0 0-.54-.15c-.32.09-.62.15-.93.26a2.5 2.5 0 0 0-.37.17q-.18.096-.33.23q-.27.257-.501.55v-.08a.37.37 0 0 0-.48-.15a.35.35 0 0 0-.15.47l.74 1.791q.084.278.21.54a.52.52 0 0 0 .36.25c.214.054.437.054.65 0a5.7 5.7 0 0 0 1-.38c.461-.19.921-.4 1.371-.63c.66-.33 1.291-.71 1.931-1.07q.673-.315 1.28-.74a.9.9 0 0 0 .3-.5a1.9 1.9 0 0 0-.11-.781q-.27-.6-.63-1.15"/>
          </g>
        </svg>
      ),
      title: t('howWeWork.design.title'),
      description: t('howWeWork.design.description'),
    },
    {
      key: 'development',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" className={styles.stepIcon}>
          <path fill="none" stroke="currentColor" strokeWidth="1.5" d="M1.165 21.304h10.5l9.5-3v-.392a2.61 2.61 0 0 0-3.375-2.493l-6.125 1.885m-5 0h5m0 0v-3l-6.29-.968a3.003 3.003 0 0 0-3.46 2.968m9.034-12.187L8.105 6.96l2.844 2.844m8.162-5.687l2.844 2.844l-2.844 2.844m-5.378.615l2.594-7.783"/>
        </svg>
      ),
      title: t('howWeWork.development.title'),
      description: t('howWeWork.development.description'),
    },
    {
      key: 'launch',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" className={styles.stepIcon}>
          <path fill="currentColor" d="M24.01 20.027v2h-24v-2h4v-1a2.006 2.006 0 0 1-2-2v-10a2.006 2.006 0 0 1 2-2h1.996v2H4.01v10h16v-10h-2.004v-2h2.004a2.006 2.006 0 0 1 2 2l-.01 10a1.997 1.997 0 0 1-1.99 2v1Zm-9-6.012l-3-3l-3 3h2v2.01h2v-2.01Zm.995-7.991a4 4 0 1 1-4-4a4 4 0 0 1 4 4m-4.4 2.96v-.56a.8.8 0 0 1-.8-.8v-.4L9.06 5.479a2.96 2.96 0 0 0 2.545 3.505m2.658-1.007a2.977 2.977 0 0 0-1.068-4.704a.797.797 0 0 1-.79.75h-.8v.8a.4.4 0 0 1-.4.4h-.8v.8h2.4a.4.4 0 0 1 .4.4v1.2h.4a.79.79 0 0 1 .658.354"/>
        </svg>
      ),
      title: t('howWeWork.launch.title'),
      description: t('howWeWork.launch.description'),
    },
  ];

  return (
    <section className={styles.howWeWork}>
      <div className={styles.container}>
        <section ref={el => { elementsRef.current[0] = el; }} className="fade-in-right">
          <h2 className="highlight">
            {t('howWeWork.title')}
          </h2>
        </section>

        <section ref={el => { elementsRef.current[1] = el; }} className="fade-in-left">
          <p className={styles.subtitle}>
            {t('howWeWork.subtitle')}
          </p>
        </section>

        <div className={styles.stepsGrid} ref={gridRef}>
          {/* Los rieles van en su PROPIA capa (no como hijos sueltos de la grilla):
              si fueran hijos directos correrían el nth-child de las tarjetas y el
              zigzag se desarma. */}
          <span className={styles.railLayer} aria-hidden>
            {/* Riel base: exactamente del primer punto al último. */}
            <span
              className={styles.railBase}
              style={{ top: `${railTop}px`, height: `${Math.max(0, railBottom - railTop)}px` }}
            />
            {/* Tramo recorrido: del primer punto hasta el nodo activo. */}
            <span
              className={styles.railProgress}
              style={{
                top: `${railTop}px`,
                height: `${Math.max(0, dotTop - railTop)}px`,
                transition: armed ? undefined : 'none',
              }}
            />

            {/* El panda NO cuelga del tramo: se posiciona por su cuenta sobre el
                riel. Si fuera hijo de .railProgress su descenso dependería del
                height del padre y su giro de su propia transición — dos
                animaciones separadas que se desincronizan. Acá una sola
                propiedad `transform` mueve y gira a la vez, con el mismo
                easing, así el giro y la bajada son literalmente el mismo
                movimiento. La rotación se deriva del paso activo (no del
                scroll): va a velocidad constante y no tiembla al scrollear
                rápido. Una vuelta entera por paso (ver pandaSpin): sólo los
                múltiplos de 360° dejan el isotipo derecho al llegar. */}
            {/* eslint-disable-next-line @next/next/no-img-element -- SVG: next/image no lo optimiza */}
            <img
              className={styles.railPanda}
              src="/isotipo-panda-contorno.svg"
              alt=""
              aria-hidden
              style={{
                transform: `translate(-50%, -50%) translateY(${dotTop}px) rotate(${pandaSpin}deg)`,
                transition: armed ? undefined : 'none',
              }}
            />
          </span>

          {steps.map((step, index) => {
            const animationClass = index % 2 === 0 ? 'fade-in-left' : 'fade-in-right';
            // Un paso "protagonista" es el que el panda alcanzó (el activo
            // actual); los ya visitados quedan como "hechos", los futuros dormidos.
            const isCurrent = index === activeStep;
            const isDone = index < activeStep;
            const stateClass = isCurrent ? styles.stepCurrent : isDone ? styles.stepDone : '';
            return (
              <div
                key={step.key}
                ref={(el) => { elementsRef.current[index + 2] = el; }}
                className={`${styles.stepCard} ${animationClass} ${stateClass}`}
              >
                <span
                  className={styles.stepNode}
                  ref={(el) => { nodeRefs.current[index] = el; }}
                  aria-hidden
                />
                <div className={styles.stepHeader}>
                  <div className={styles.iconWrapper}>
                    {step.icon}
                  </div>
                </div>
                <h3 className={styles.stepTitle}>{step.title}</h3>
                <p className={styles.stepDescription}>{step.description}</p>
              </div>
            );
          })}
        </div>

        {/* CTA Button Section */}
        <section ref={el => { elementsRef.current[6] = el; }} className="fade-in-right">
          <div className={styles.ctaContainer}>
            <Link href="/contact" className={styles.ctaButton}>
              {t('howWeWork.cta.text')}
              <HiArrowRight className={styles.arrow} />
            </Link>
          </div>
        </section>
      </div>
    </section>
  );
};

export default HowWeWork;

